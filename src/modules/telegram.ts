...
import { getOrCreateUserAndProfileForTelegram, getProfileTextForUserByTelegramChat, updateProfileFromTextForTelegram, saveResumeForTelegramUser, getLatestResumePathForTelegramUser, setCoreLinkForTelegramUser, addCustomLinkForTelegramUser, getLinksForTelegramUser, upsertEmailAccountForTelegramUser, getEmailAccountForTelegramUser } from '../data/profile.js';
import fs from 'fs';
...
async function startSetProfile(ctx: MyContext) {
  ...
  const keyboard = new InlineKeyboard().text("👀 View sample profile", "view_sample_profile");
...
async function startSetLinks(ctx: MyContext) {
  ...
  const keyboard = new InlineKeyboard()
    ...
    .text("LinkedIn", "setlink_linkedin").row()
...
// Session middleware
bot.use(session({ initial: (): SessionData => ({ awaitingResumeName: false, awaitingRevision: false, currentActionId: null, awaitingProfileText: false, awaitingResumeUpload: false, awaitingLinkType: null, awaitingCustomLinkLabel: false, awaitingEmailAddress: false, awaitingEmailPassword: false }) }));

const pendingEmails = new Map<string, { jobData: ParsedJobDescription, match: any, draft: EmailDraft, customResumeName?: string, coverLetterPath?: string, userId: number }>();
...
bot.command('start', async (ctx) => {
  ...
  text += "Before we start applying on your behalf, please complete these quick steps:\n\n";
  ...
  const keyboard = new InlineKeyboard()
    .text(emailAccount ? "✅ Email" : "1️⃣ Set email", "onboard_email").row()
    .text(resumePath ? "✅ Resume" : "2️⃣ Upload resume", "onboard_resume").row()
    .text("3️⃣ Set profile", "onboard_profile").row()
...
bot.command('job_hunt', async (ctx) => {
  ...
  ctx.reply("⏸ Auto-apply is currently disabled while we upgrade the system for multi-user support.");
});
...
bot.on('message:document', async (ctx) => {
  ...
  if (!file.file_path) {
    await ctx.reply("❌ I couldn't access the file path from Telegram. Please try again.");
    return;
  }
  ...
  await ctx.reply("✅ Resume uploaded and saved! I’ll attach this file for future applications that require a resume.");
});
...
bot.on('message:text', async (ctx) => {
  ...
  if (ctx.session.awaitingEmailPassword && ctx.session.currentActionId) {
    ...
    await ctx.reply("✅ Email account connected! I’ll now send applications using this address.");
    return;
  }
  ...
  if (ctx.session.awaitingProfileText) {
    ...
    await ctx.reply("✅ Profile updated! I’ll use this new information for future match scores, emails and cover letters.");
    return;
  }
  ...
  if (ctx.session.awaitingLinkType) {
    ...
    if (!/^https?:\/\//i.test(url)) {
      await ctx.reply("⚠️ Please send a valid URL starting with http:// or https://");
      return;
    }
    ...
    if (kind === 'github' || kind === 'linkedin' || kind === 'portfolio') {
      ...
      const labelPretty = kind === 'github' ? 'GitHub' : kind === 'linkedin' ? 'LinkedIn' : 'Portfolio';
...
bot.callbackQuery(/^setlink_(github|linkedin|portfolio|custom)$/, async (ctx) => {
  ...
  if (kind === 'custom') {
    ...
    await ctx.reply("📝 What label should I use for this link? (e.g. 'Blog', 'Behance', 'LeetCode')");
  } else {
    ctx.session.awaitingLinkType = kind;
    await ctx.reply(`🔗 Send me your ${kind} URL (starting with http:// or https://).`);
  }
...
bot.callbackQuery('view_sample_profile', async (ctx) => {
  const sample =
    "Here’s a sample profile you can use as a template:\n\n" +
    "Summary:\n" +
...
bot.callbackQuery(/^send_(.+)$/, async (ctx) => {
  try {
    ...
    if (!ctx.from) {
      await ctx.answerCallbackQuery({ text: "Cannot resolve your user account. Try again.", show_alert: true });
      return;
    }
    ...
    if (!emailAccount) {
      await ctx.answerCallbackQuery({ text: "You need to configure your email first via /set_email.", show_alert: true });
      return;
    }
    ...
    if (!resumePath && fs.existsSync(path.resolve(process.cwd(), 'resume.pdf'))) {
    ...
    await ctx.editMessageText(`🚀 Sending email to ${pending.jobData.applicationEmail} from ${emailAccount.email_address}...`);
